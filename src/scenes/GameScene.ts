...
export class GameScene extends Phaser.Scene {
  ...
  init(data: any): void {
    ...
    if (data.equippedItems) {
      ...
      if (data.equippedItems.specialSlot) {
        ...
        Logger.debug(
          "特殊枠アイテム追加後の所持数:",
          this.itemManager.getItemCount(data.equippedItems.specialSlot.id)
        );
        const success1 = this.itemManager.equipItem(
          data.equippedItems.specialSlot,
          "special"
        );
      ...
      if (data.equippedItems.normalSlot) {
        ...
        Logger.debug(
          "通常枠アイテム追加後の所持数:",
          this.itemManager.getItemCount(data.equippedItems.normalSlot.id)
        );
        const success2 = this.itemManager.equipItem(
          data.equippedItems.normalSlot,
          "normal"
        );
  ...
  create(): void {
    ...
    if (!this.textures.exists("rockBlockTexture")) {
      ...
      graphics.moveTo(
        GameConfig.BLOCK_SIZE / 2 - size / 2,
        GameConfig.BLOCK_SIZE / 2 - size / 3
      );
      graphics.lineTo(
        GameConfig.BLOCK_SIZE / 2 + size / 2,
        GameConfig.BLOCK_SIZE / 2 - size / 3
    ...
    const headerText = this.add
      .text(10, 15, `Stage ${this.currentStage}  Score: ${this.score}`, {
        // 30から15に調整
        fontSize: "18px",
        color: "#FFFFFF",
        stroke: "#000000",
    ...
    const targetText = this.add.text(10, 40, `Target: ${this.targetScore}`, {
      // 60から40に調整
      fontSize: "16px",
      color: "#FFFFFF",
      stroke: "#000000",
    ...
    Logger.debug(
      `ボタン座標計算: height=${height}, buttonHeight=${buttonHeight}, buttonCenterY=${buttonCenterY}`
    );
    ...
    const itemButtons = this.children.list.filter(
      (child) => child.name && child.name.startsWith("itemButton_")
    );
  ...
  private createInitialBlocks(): void {
    // ブロック配列の初期化
    this.blocks = Array(GameConfig.BOARD_HEIGHT)
      .fill(0)
      .map(() => Array(GameConfig.BOARD_WIDTH).fill(null));

    this.blockSprites = Array(GameConfig.BOARD_HEIGHT)
      .fill(0)
      .map(() => Array(GameConfig.BOARD_WIDTH).fill(null));
    ...
    if (this.currentStageConfig?.obstacles) {
      this.currentStageConfig.obstacles.forEach((obstacle) => {
        if (
          obstacle.y < GameConfig.BOARD_HEIGHT &&
          obstacle.x < GameConfig.BOARD_WIDTH
        ) {
          let block: Block;

          switch (obstacle.type) {
            case "iceLv1":
              block = blockFactory.createIceBlockLv1(
                obstacle.x,
                obstacle.y,
                obstacle.color || availableColors[0]
              );
              break;
            case "iceLv2":
              block = blockFactory.createIceBlockLv2(
                obstacle.x,
                obstacle.y,
                obstacle.color || availableColors[0]
              );
              break;
            case "counterPlus":
              block = blockFactory.createCounterPlusBlock(
                obstacle.x,
                obstacle.y,
                obstacle.color || availableColors[0],
                obstacle.counter || 3
              ...
              break;
            case "counterMinus":
              block = blockFactory.createCounterMinusBlock(
                obstacle.x,
                obstacle.y,
                obstacle.color || availableColors[0],
                obstacle.counter || 3
              ...
              break;
            case "iceCounterPlus":
              block = blockFactory.createIceCounterPlusBlock(
                obstacle.x,
                obstacle.y,
                obstacle.color || availableColors[0],
                obstacle.counter || 3
            ...
            case "iceCounterMinus":
              block = blockFactory.createIceCounterMinusBlock(
                obstacle.x,
                obstacle.y,
                obstacle.color || availableColors[0],
                obstacle.counter || 3
            ...
            default:
              ...
              const color =
                GameConfig.BLOCK_COLORS[
                  colorKey as keyof typeof GameConfig.BLOCK_COLORS
                ];
              block = blockFactory.createNormalBlock(
                obstacle.x,
                obstacle.y,
                color
              );
  ...
  private createBlockSprite(
    ...
    const blockX =
      this.boardX + x * GameConfig.BLOCK_SIZE + GameConfig.BLOCK_SIZE / 2;
    const blockY =
      this.boardY + y * GameConfig.BLOCK_SIZE + GameConfig.BLOCK_SIZE / 2;

    // 海をテーマにした美しいブロックを作成
    const blockContainer = this.createOceanThemedBlock(
      blockX,
      blockY,
      block.color,
      block.type,
    ...
    // スプライト配列に保存（型変換が必要）
    this.blockSprites[y][x] =
      blockContainer as unknown as Phaser.GameObjects.Sprite;

    // ブロックオブジェクトにスプライト参照を追加
    this.blocks[y][x].sprite =
      blockContainer as unknown as Phaser.GameObjects.Sprite;
   ...
   * コンテナにホバーエフェクトを追加
   */
  private addContainerHoverEffect(
    ...
    container.on("pointerover", () => {
      ...
      const blockX = Math.floor(
        (container.x - this.boardX) / GameConfig.BLOCK_SIZE
      );
      const blockY = Math.floor(
        (container.y - this.boardY) / GameConfig.BLOCK_SIZE
      );
      ...
      if (connectedBlocks.length >= 2) {
        ...
        connectedBlocks.forEach((pos) => {
          ...
          if (targetSprite) {
            ...
            if (
              targetBlock &&
              (targetBlock.type === BlockType.NORMAL ||
                targetBlock.type === "normal")
            ) {
              this.tweens.add({
                targets: targetSprite,
  ...
  private addBlockHoverEffect(
    ...
    sprite.on("pointerover", () => {
      ...
      // ブロックの座標を取得
      const blockX = Math.floor(
        (sprite.x - this.boardX) / GameConfig.BLOCK_SIZE
      );
      const blockY = Math.floor(
        (sprite.y - this.boardY) / GameConfig.BLOCK_SIZE
      );
   ...
   * 海をテーマにした美しいブロックを作成
   */
  private createOceanThemedBlock(
    x: number,
    y: number,
    color: string,
    ...
    graphics.fillGradientStyle(
      baseColor,
      baseColor,
      this.getLighterColor(baseColor),
      this.getLighterColor(baseColor),
      0.9,
      0.9,
      0.6,
  ...
  private addSteelEffect(
    ...
    // 鋼鉄の背景
    steelGraphics.fillGradientStyle(
      0xc0c0c0,
      0xc0c0c0,
      0x808080,
      0x808080,
    ...
    steelGraphics.strokeRoundedRect(
      -radius,
      -radius,
      radius * 2,
      radius * 2,
  ...
  private handleItemModeClick(x: number, y: number): void {
    ...
    const item =
      this.selectedItemSlot === "special"
        ? equippedItems.specialSlot
        : equippedItems.normalSlot;
    ...
    switch (item.effectType) {
      ...
      case "miniBomb":
        ...
        if (result && result.success) {
          ...
          // パーティクルエフェクトを追加
          this.particleManager.createItemUseEffect(
            centerX,
            centerY,
            "miniBomb"
          );
  ...
  private findAdjacentBlock(
    ...
    for (const dir of directions) {
      ...
      if (
        newX >= 0 &&
        newX < GameConfig.BOARD_WIDTH &&
        newY >= 0 &&
        newY < GameConfig.BOARD_HEIGHT &&
  ...
  private handleNormalBlockClick(x: number, y: number): void {
    try {
      ...
      if (
        clickedBlock.type === BlockType.ICE_LV1 ||
        ...
        // 妨害ブロックは直接クリックできない
        this.errorHandler.handleItemUseError(
          "ブロック",
          "妨害ブロックは直接クリックできません"
        );
        return;
      ...
      // 通常ブロックの処理
      // 隣接する同色ブロックを検索
      const connectedBlocks = this.blockLogic.findConnectedBlocks(
        this.blocks,
        x,
        y
      );
      ...
      if (connectedBlocks.length >= 2) {
        ...
        const removableCount =
          connectedBlocks.length - nonRemovableCounterBlocks.length;
        ...
        // 氷結ブロックの状態更新（レベルダウン）
        this.blocks = this.blockLogic.updateIceBlocks(
          this.blocks,
          connectedBlocks
        );

        // デバッグ用：氷結ブロック更新後の状態を記録
        BlockAsciiRenderer.logBlocksComparison(
          beforeIceUpdate,
          this.blocks,
          `氷結ブロック更新 (${String.fromCharCode(97 + x)}${y})`,
          { x, y }
        );
        ...
        for (const block of connectedBlocks) {
          ...
          if (
            beforeIceUpdate[block.y][block.x].type === BlockType.NORMAL ||
        ...
        this.time.delayedCall(100, () => {
          BlockAsciiRenderer.logBlocksComparison(
            beforeRemove,
            this.blocks,
            `ブロック消去 (${String.fromCharCode(97 + x)}${y})`,
            { x, y }
          );
        });
        ...
        this.time.delayedCall(GameConfig.ANIMATION.PROCESSING_DELAY, () => {
          ...
          BlockAsciiRenderer.logBlocksComparison(
            beforeGravity,
            this.blocks,
            `重力適用 (${String.fromCharCode(97 + x)}${y})`,
            { x, y }
          );
          ...
          BlockAsciiRenderer.logBlocksComparison(
            beforeBlocks,
            this.blocks,
            `全体処理 (${String.fromCharCode(97 + x)}${y})`,
            { x, y }
          );
  ...
  private removeBlocks(blocks: Block[]): void {
    ...
    blocks.forEach((block) => {
      ...
      if (
        block &&
        block.y >= 0 &&
        block.y < this.blocks.length &&
        block.x >= 0 &&
        block.x < this.blocks[block.y].length
      ) {
      ...
      if (block && block.sprite) {
        ...
        const worldX =
          this.boardX +
          block.x * GameConfig.BLOCK_SIZE +
          GameConfig.BLOCK_SIZE / 2;
        const worldY =
          this.boardY +
          block.y * GameConfig.BLOCK_SIZE +
          GameConfig.BLOCK_SIZE / 2;
        ...
        if (
          block.y >= 0 &&
          block.y < this.blockSprites.length &&
          block.x >= 0 &&
          block.x < this.blockSprites[block.y].length
        ) {
          this.blockSprites[block.y][block.x] = null;
        }

        // カウンターテキストがある場合は削除
        const counterText = sprite.getData(
          "counterText"
        ...
        // 星グラフィックスがある場合は削除
        const starGraphics = sprite.getData(
          "starGraphics"
  ...
  private updateBlockSprites(): void {
    ...
    const boardRight =
      this.boardX + GameConfig.BOARD_WIDTH * GameConfig.BLOCK_SIZE;
    const boardTop = this.boardY;
    const boardBottom =
      this.boardY + GameConfig.BOARD_HEIGHT * GameConfig.BLOCK_SIZE;
    ...
    this.children.list.slice().forEach((child) => {
      ...
      if ("x" in child && "y" in child) {
        ...
        if (
          gameObject.x >= boardLeft &&
          gameObject.x <= boardRight &&
          gameObject.y >= boardTop &&
          gameObject.y <= boardBottom &&
          gameObject.name !== "boardBackground"
        ) {
          // 背景は削除しない
          child.destroy();
          removedCount++;
    ...
    this.blockSprites = Array(this.blocks.length)
      .fill(0)
      .map(() =>
    ...
    for (let y = 0; y < this.blocks.length; y++) {
      for (let x = 0; x < this.blocks[y].length; x++) {
        ...
        const hasVisual =
          this.blockSprites[y] && this.blockSprites[y][x] !== null;
        if (hasLogic !== hasVisual) {
          Logger.warn(
            `⚠️ 不整合検出: (${x}, ${y}) 論理=${hasLogic}, 視覚=${hasVisual}`
          );
          syncIssues++;
        }
      }
  ...
  private updateScoreDisplay(): void {
    // ヘッダーテキストを更新
    const headerText = this.children.getByName(
      "headerText"
    ) as Phaser.GameObjects.Text;
    if (headerText) {
  ...
  private showAllClearedEffect(): void {
    ...
    // 全消しテキスト
    const allClearedText = this.add
      .text(width / 2, height / 2, "全消しボーナス！", {
        fontSize: "32px",
        color: "#FFFF00",
        stroke: "#000000",
        strokeThickness: 4,
  ...
  private showNoMovesEffect(): void {
    ...
    // 行き詰まりテキスト
    const noMovesText = this.add
      .text(width / 2, height / 2, "消去可能なブロックがありません", {
        fontSize: "20px",
        color: "#FFFFFF",
        stroke: "#000000",
        strokeThickness: 3,
  ...
  private showClearButton(): void {
    ...
    const retireButton = this.children.getByName(
      "retireButton"
    ) as SimpleOceanButton;
  ...
  private createItemButtons(buttonY: number): void {
    ...
    if (equippedItems.specialSlot) {
      ...
      this.createItemButton(
        equippedItems.specialSlot.name,
        "special",
        leftButtonX,
        buttonY
      );
  ...
  private createItemButton(
    ...
    Logger.debug(
      `createItemButton呼び出し: ${itemName}, slot: ${slot}, x: ${x}, y: ${y}`
    );
    ...
    Logger.debug(
      `ボタン作成完了: ${button.name}, 座標: (${button.x}, ${button.y})`
    );
    Logger.debug(
      `テキスト作成完了: ${text.name}, テキスト: "${itemName}", 座標: (${text.x}, ${text.y})`
    );
  ...
  private onItemButtonClick(slot: "special" | "normal"): void {
    ...
    const equippedItems = this.itemManager.getEquippedItems();
    const item =
      slot === "special" ? equippedItems.specialSlot : equippedItems.normalSlot;
  ...
  private showItemModeUI(): void {
    ...
    if (item) {
      ...
      const message = this.add
        ...
        .setOrigin(0.5)
        .setName("itemModeMessage");

      // キャンセルボタン
      const cancelButton = this.add
        .rectangle(width - 50, 100, 80, 30, 0xaa2222)
        .setInteractive({ useHandCursor: true })
        .setName("itemCancelButton");

      const cancelText = this.add
        .text(width - 50, 100, "キャンセル", {
          fontSize: "12px",
          color: "#FFFFFF",
        })
        .setOrigin(0.5)
        .setName("itemCancelText");
  ...
  private exitItemMode(): void {
    ...
    if (this.swapFirstBlock) {
      this.highlightSwapBlock(
        this.swapFirstBlock.x,
        this.swapFirstBlock.y,
        false
      );
      this.resetSwapState();
    }
   ...
   * スコア獲得エフェクトを表示
   */
  private showScoreGainEffect(
    score: number,
    blockX: number,
    blockY: number
  ): void {
    const screenX =
      this.boardX + blockX * GameConfig.BLOCK_SIZE + GameConfig.BLOCK_SIZE / 2;
    const screenY =
      this.boardY + blockY * GameConfig.BLOCK_SIZE + GameConfig.BLOCK_SIZE / 2;
   ...
   */
  private getConnectedBlocksForPreview(
    startX: number,
    startY: number
  ): { x: number; y: number }[] {
   ...
   * 色選択UIを表示
   */
  private showColorSelectionUI(
    x: number,
    y: number,
    itemType: "changeOne" | "changeArea"
  ): void {
    const { width, height } = this.cameras.main;
    ...
    const modal = this.add
      .rectangle(width / 2, height / 2, modalWidth, modalHeight, 0x333333, 0.95)
    ...
    // タイトル
    const title =
      itemType === "changeOne" ? "ブロックの色を選択" : "エリアの色を選択";
    this.add
      .text(width / 2, height / 2 - modalHeight / 2 + 30, title, {
        fontSize: "16px",
        color: "#FFFFFF",
        fontFamily: "Arial",
      })
    ...
    colors.forEach((colorData, index) => {
      ...
      const colorButton = this.add
        .rectangle(buttonX, buttonY, buttonSize, buttonSize, colorData.hex)
        .setStrokeStyle(2, 0xffffff)
        .setInteractive({ useHandCursor: true })
        .setDepth(1002)
      ...
      // 色名ラベル
      this.add
        .text(buttonX, buttonY + buttonSize / 2 + 15, colorData.name, {
          fontSize: "10px",
          color: "#FFFFFF",
          fontFamily: "Arial",
        })
    ...
    const closeButton = this.add
      .rectangle(width / 2, height / 2 + modalHeight / 2 - 20, 80, 25, 0x9e9e9e)
      .setInteractive({ useHandCursor: true })
      .setDepth(1002)
      .setName("colorSelectionCloseButton");

    this.add
      .text(width / 2, height / 2 + modalHeight / 2 - 20, "閉じる", {
        fontSize: "12px",
        color: "#FFFFFF",
        fontFamily: "Arial",
      })
      .setOrigin(0.5)
      .setDepth(1002)
      .setName("colorSelectionCloseText");
  ...
  private executeSwap(
    ...
    Logger.debug(
      `スワップ実行: (${pos1.x}, ${pos1.y}) <-> (${pos2.x}, ${pos2.y})`
    );
  ...
  private updateItemModeMessage(message: string): void {
    // 既存のメッセージテキストを更新
    const messageText = this.children.getByName(
      "itemModeMessage"
    ) as Phaser.GameObjects.Text;
    if (messageText) {
   ...
   * 演出付きでブロックスプライトを作成
   */
  private createBlockSpriteWithAnimation(
    x: number,
    y: number,
    block: Block
  ): void {
