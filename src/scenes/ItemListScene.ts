...
export class ItemListScene extends Phaser.Scene {
  ...
  private createUI(): void {
    ...
    const titleY = 40;
    const titleText = this.add.text(width / 2, titleY, 'アイテム一覧', {
      fontSize: '28px',
      color: '#FFFFFF',
      fontFamily: 'Arial',
      stroke: '#000000',
  ...
  private createItemList(): void {
    ...
    if (sortedItems.length === 0) {
      const noItemsText = this.add.text(width / 2, height / 2, 'アイテムを所持していません\nガチャでアイテムを獲得しましょう！', {
        fontSize: '18px',
        color: '#CCCCCC',
        fontFamily: 'Arial',
        align: 'center'
    ...
    sortedItems.forEach((item, index) => {
      ...
      const card = this.createItemCard(x, y, cardWidth, item, index * animationDelay);
    });
  }

  private createItemCard(x: number, y: number, cardWidth: number, item: any, animationDelay: number): Phaser.GameObjects.Container {
    ...
    // アイテム名（左側）
    const nameText = this.add.text(-cardWidth / 2 + 10, -8, item.data?.name || item.itemId, {
      fontSize: '14px',
      color: '#FFFFFF',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    // レア度表示（左側、アイテム名の下）
    const rarityText = this.add.text(-cardWidth / 2 + 10, 8, `${item.rarity}レア`, {
      fontSize: '10px',
    ...
    // 所持数表示（右下）- 背景を黒に変更
    const countBg = this.add.rectangle(cardWidth / 2 - 25, 13, 40, 20, 0x000000, 0.8);
    countBg.setStrokeStyle(1, 0xFFFFFF, 0.8);
    card.add(countBg);
    
    const countText = this.add.text(cardWidth / 2 - 25, 13, `×${item.count}`, {
      fontSize: '11px',
      color: '#FFFFFF',
      fontFamily: 'Arial',
      fontStyle: 'bold'
  ...
  private getRarityColorHex(rarity: string): number {
    switch (rarity) {
      ...
      case 'D': return 0x00FF00; // 緑色
      case 'E': return 0xFFFFFF; // 白色
      case 'F': return 0x808080; // 灰色
  ...
  private addDebugLines(): void {
    ...
    this.debugHelper.addAreaBorder(width / 2, 120 + contentHeight / 2, width - 40, contentHeight, 0x0000FF, 'メインコンテンツエリア');
