...
export class GachaResultScene extends Phaser.Scene {
  ...
  private createGoldDisplay(): void {
    ...
    // ゴールド数値
    this.add
      .text(width - 95, 40, `${currentGold.toLocaleString()}G`, {
        fontSize: "14px",
        color: "#FFD700",
        fontFamily: "Arial",
        fontStyle: "bold",
  ...
  private createSingleResult(): void {
    ...
    // アイテム名
    this.add
      .text(width / 2, itemY - 20, item.itemName, {
        fontSize: "20px",
        color: "#FFFFFF",
        fontFamily: "Arial",
        stroke: "#000000",
    ...
    // レア度表示
    this.add
      .text(width / 2, itemY + 10, `${item.rarity}レア`, {
        fontSize: "16px",
    ...
    this.add
      .text(width / 2, itemY + 70, description, {
        fontSize: "12px",
        color: "#CCCCCC",
        fontFamily: "Arial",
        wordWrap: { width: 300 },
  ...
  private createMultiResult(): void {
    ...
    // レアアイテムがあるかチェック
    const hasRareItem = this.resultItems.some(
      (item) =>
    ...
    const totalGridWidth =
      itemsPerRow * itemWidth + (itemsPerRow - 1) * spacingX;
    ...
    // 合計表示（集計表示エリア内に配置）
    this.add
      .text(width / 2, height - 150, `合計: ${this.resultItems.length}個`, {
        fontSize: "16px",
        color: "#FFFFFF",
        fontFamily: "Arial",
      })
  ...
  private createRaritySummary(): void {
    ...
    this.add
      .text(width / 2, summaryY, summaryText, {
        fontSize: "14px",
        color: "#CCCCCC",
        fontFamily: "Arial",
      })
  ...
  private createButtons(): void {
    ...
    // 現在のゴールドと必要コストを取得
    const currentGold = this.stageManager.getCurrentGold();
    const requiredCost = this.isMulti
      ? this.gachaManager.getTenGachaPrice()
  ...
  private onBack(): void {
    ...
    this.animationManager
      .screenTransition("GachaResultScene", "GachaScene", TransitionType.BUBBLE)
      .then(() => {
        this.scene.start("GachaScene");
      });
  }
