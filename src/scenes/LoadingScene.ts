...
export class LoadingScene extends Phaser.Scene {
  ...
  private createLoadingUI(): void {
    ...
    // タイトル
    this.add.text(width / 2, height / 2 - 100, 'さめがめオーシャン', {
      fontSize: '32px',
      color: '#FFFFFF',
      fontFamily: 'Arial',
      stroke: '#1E5799',
    ...
    this.progressBox.fillRoundedRect(width / 2 - 160, height / 2 + 50, 320, 20, 10);
  ...
  private updateProgress(value: number): void {
    ...
    this.progressBar.fillRoundedRect(width / 2 - 160, this.cameras.main.height / 2 + 50, barWidth, 20, 10);
    
    // パーセンテージ表示
    const percentText = `${Math.round(value * 100)}%`;
    if (this.children.getByName('percentText')) {
      (this.children.getByName('percentText') as Phaser.GameObjects.Text).setText(percentText);
    } else {
      this.add.text(width / 2, this.cameras.main.height / 2 + 25, percentText, {
        fontSize: '14px',
        color: '#FFFFFF',
        fontFamily: 'Arial'
