...
export class TitleScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const startButton = this.add
      .rectangle(width / 2, height / 2, 200, 60, 0x0088ff)
      .setInteractive({ useHandCursor: true });

    const startText = this.add
      .text(width / 2, height / 2, "ゲーム開始", {
        fontSize: "24px",
        color: "#FFFFFF",
      })
      .setOrigin(0.5);
    ...
    const versionText = this.add
      .text(width / 2, height - 20, "Ver 1.0.0", {
        fontSize: "12px",
        color: "#FFFFFF",
      })
      .setOrigin(0.5);
  ...
  private addDebugLines(): void {
    ...
    if (middleTopSpaceHeight > 0) {
      this.debugHelper.addAreaBorder(
        width / 2,
        height / 4 + 80 / 2 + middleTopSpaceHeight / 2,
        width,
