...
export class ItemSelectionScene extends Phaser.Scene {
  ...
  private loadAvailableItems(): void {
    ...
    for (const itemId in inventory) {
      ...
      if (itemData && inventory[itemId] > 0) {
        ...
        Logger.debug(
          `アイテム追加: ${itemData.name} (${itemId}) x${inventory[itemId]}`
        );
      }
    }
    ...
    Logger.debug(
      "ソート後のアイテム一覧:",
      this.availableItems.map((item) => `${item.name}(${item.rarity})`)
    );
  }
  ...
  private createUI(): void {
    ...
    // ステージ情報
    this.add
      .text(width / 2, titleY + 30, `ステージ ${this.currentStage}`, {
        fontSize: "18px",
        color: "#FFFF00",
        fontFamily: "Arial",
      })
  ...
  private createItemSelection(): void {
    ...
    // 特殊枠（S・Aレア用）
    this.add
      .text(width / 2, contentStartY, "◆特殊枠（S・Aレア用）", {
        fontSize: "16px",
        color: "#FFD700",
        fontFamily: "Arial",
      })
    ...
    const specialSlotRect = this.add
      .rectangle(width / 2, specialSlotY, slotWidth, slotHeight, 0x333333, 0.8)
      .setStrokeStyle(2, 0xffd700)
      .setInteractive({ useHandCursor: true })
      .setName("specialSlot");

    const specialSlotText = this.add
      .text(width / 2, specialSlotY, "未選択", {
        fontSize: "14px",
        color: "#FFFFFF",
        fontFamily: "Arial",
      })
      .setOrigin(0.5)
      .setName("specialSlotText");

    // 特殊枠の装備解除ボタン
    const specialUnequipButton = this.add
      .text(width / 2 + slotWidth / 2 + 20, specialSlotY, "×", {
        fontSize: "16px",
        color: "#FF0000",
        fontFamily: "Arial",
      })
    ...
    this.add
      .text(width / 2, normalFrameY, "◆通常枠（B〜Fレア用）", {
        fontSize: "16px",
        color: "#87CEEB",
        fontFamily: "Arial",
      })
    ...
    const normalSlotRect = this.add
      .rectangle(width / 2, normalSlotY, slotWidth, slotHeight, 0x333333, 0.8)
      .setStrokeStyle(2, 0x87ceeb)
      .setInteractive({ useHandCursor: true })
      .setName("normalSlot");

    const normalSlotText = this.add
      .text(width / 2, normalSlotY, "未選択", {
        fontSize: "14px",
        color: "#FFFFFF",
        fontFamily: "Arial",
      })
      .setOrigin(0.5)
      .setName("normalSlotText");

    // 通常枠の装備解除ボタン
    const normalUnequipButton = this.add
      .text(width / 2 + slotWidth / 2 + 20, normalSlotY, "×", {
        fontSize: "16px",
        color: "#FF0000",
        fontFamily: "Arial",
      })
    ...
    this.add
      .text(width / 2, itemListY, "◆所持アイテム一覧", {
        fontSize: "16px",
        color: "#FFFFFF",
        fontFamily: "Arial",
      })
  ...
  private createItemList(startY: number): void {
    ...
    if (this.availableItems.length === 0) {
      this.add
        .text(width / 2, startY, "アイテムを所持していません", {
          fontSize: "14px",
          color: "#888888",
          fontFamily: "Arial",
        })
    ...
    const fontSize =
      itemHeight >= 30 ? "14px" : itemHeight >= 25 ? "13px" : "12px";
    ...
    this.availableItems.forEach((item, index) => {
      ...
      const itemBg = this.add
        .rectangle(x, y, columnWidth - 10, itemHeight - 2, 0x000000, 0.1)
        .setInteractive({ useHandCursor: true })
  ...
  private showItemSelectionModal(slotType: "special" | "normal"): void {
    ...
    this.add
      .text(
        width / 2,
        height / 2 - modalHeight / 2 + titleHeight / 2,
        `${slotTypeName}に装備するアイテムを選択`,
        {
          fontSize: "16px",
          color: "#FFFFFF",
          fontFamily: "Arial",
    ...
    equipableItems.forEach((item, index) => {
      ...
      const itemText = `${item.name} ×${count} (${item.rarity})`;
      this.add
        .text(width / 2, itemY, itemText, {
          fontSize: fontSize,
          color: rarityColor,
          fontFamily: "Arial",
        })
        .setOrigin(0.5)
        .setName(`modalItemText_${item.id}`);
    ...
    // 閉じるボタン
    const closeButtonY =
      height / 2 + modalHeight / 2 - buttonHeight / 2 - padding / 2;
    const closeButton = this.add
      .rectangle(width / 2, closeButtonY, 100, 30, 0x9e9e9e)
      .setInteractive({ useHandCursor: true })
      .setName("modalCloseButton");

    this.add
      .text(width / 2, closeButtonY, "閉じる", {
        fontSize: "14px",
        color: "#FFFFFF",
        fontFamily: "Arial",
      })
      .setOrigin(0.5)
      .setName("modalCloseText");
  ...
  private updateSlotDisplay(textName: string, item: Item | null): void {
    ...
    const unequipButton = this.children.getByName(
      unequipButtonName
  ...
  private unequipItem(slotType: "special" | "normal"): void {
    ...
    console.log(
      `${slotType === "special" ? "特殊枠" : "通常枠"}のアイテムを解除しました`
    );
  }
  ...
  private autoEquipTestItems(): void {
    ...
    // 利用可能なアイテムから自動装備
    const availableItems = this.availableItems.filter(
      (item) => this.itemManager.getItemCount(item.id) > 0
    );
    ...
    if (availableItems.length > 0) {
      ...
      if (success1) {
        ...
        console.log(
          `自動装備失敗: ${firstItem.name}を特殊枠に装備できませんでした`
        );
      }
      ...
      if (availableItems.length > 1) {
        ...
        if (["B", "C", "D", "E", "F"].includes(secondItem.rarity)) {
          ...
          if (success2) {
            ...
            console.log(
              `自動装備失敗: ${secondItem.name}を通常枠に装備できませんでした`
            );
          }
        } else {
          console.log(
            `${secondItem.name}は通常枠に装備できません（レア度: ${secondItem.rarity}）`
          );
          // B〜Fレアのアイテムを探す
          const normalSlotItem = availableItems.find((item) =>
            ["B", "C", "D", "E", "F"].includes(item.rarity)
          );
          if (normalSlotItem) {
            ...
            if (success2) {
              ...
              console.log(
                `自動装備失敗: ${normalSlotItem.name}を通常枠に装備できませんでした`
              );
            }
          } else {
