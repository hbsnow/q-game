...
export class GachaScene extends Phaser.Scene {
  ...
  private createGachaContent(): void {
    ...
    // ガチャ名
    this.add
      .text(width / 2, contentY, "オーシャンガチャ", {
        fontSize: "20px",
        color: "#FFFFFF",
        fontFamily: "Arial",
      })
    ...
    this.add
      .text(width / 2, contentY + 30, `(${singleCost}G)`, {
        fontSize: "16px",
        color: "#FFFF00",
        fontFamily: "Arial",
      })
  ...
  private showRateDisplay(): void {
    ...
    // パネルに海のテーマの装飾を追加
    const decorBg = this.add
      .rectangle(width / 2, height / 2, width - 50, height - 110, 0x1e40af, 0.3)
      .setName("rateDecor");

    // タイトル（美しいスタイル）
    const titleText = this.add
      .text(width / 2, 80, "🎰 排出確率 🎰", {
        fontSize: "22px",
        color: "#FFFFFF",
        fontFamily: "Arial",
        fontStyle: "bold",
    ...
    this.dropRates.forEach((rarityData, index) => {
      ...
      const cardBg = this.add
        .rectangle(width / 2, yOffset + 15, width - 80, 30, rarityColor, 0.4)
        .setStrokeStyle(2, rarityColor, 0.8)
        .setName("rateItem");

      // レア度ヘッダー（改善されたスタイル）
      const rarityHeader = this.add
        .text(
          width / 2,
          yOffset + 15,
      ...
      if (itemsText.length > 0 && yOffset < height - 120) {
        this.add
          .text(width / 2, yOffset, itemsText, {
            fontSize: "12px",
            color: "#E5E7EB",
            fontFamily: "Arial",
  ...
  private onBack(): void {
    ...
    const backButton = this.children.list.find(
      (child) =>
    ...
    if (backButton) {
      this.animationManager.buttonClick(backButton, () => {
        ...
        this.animationManager
          .screenTransition("GachaScene", "MainScene", TransitionType.BUBBLE)
          .then(() => {
  ...
  private createGoldDisplay(): void {
    ...
    // ゴールド数値
    this.add
      .text(width - 95, 40, `${this.currentGold.toLocaleString()}G`, {
        fontSize: "14px",
        color: "#FFD700",
        fontFamily: "Arial",
        fontStyle: "bold",
