...
export class ItemEffectManager {
  ...
  static applySwap(
    ...
    // 座標の有効性チェック
    if (
      !this.isValidPosition(pos1, blocks) ||
      !this.isValidPosition(pos2, blocks)
    ) {
    ...
    if (this.isUnswappableBlock(block1) || this.isUnswappableBlock(block2)) {
      return {
        success: false,
        message: "岩ブロックと鋼鉄ブロックは入れ替えできません",
      };
    }
    ...
    // 座標を更新して入れ替え
    newBlocks[pos1.y][pos1.x] = {
      ...newBlocks[pos2.y][pos2.x]!,
      x: pos1.x,
      y: pos1.y,
      sprite: null as any,
    };
  ...
  static applyShuffle(blocks: Block[][]): ItemEffectResult {
    ...
    normalBlockPositions.forEach((pos, index) => {
      if (
        newBlocks[pos.y][pos.x] &&
        newBlocks[pos.y][pos.x]!.type === "normal"
      ) {
        newBlocks[pos.y][pos.x] = {
          ...newBlocks[pos.y][pos.x]!,
  ...
  private static isValidPosition(
    ...
    if (blocks) {
      ...
      return (
        pos.x >= 0 &&
        pos.x < GameConfig.BOARD_WIDTH &&
        pos.y >= 0 &&
        pos.y < GameConfig.BOARD_HEIGHT
      );
    }
  }
  ...
  private static deepCopyBlocks(blocks: Block[][]): Block[][] {
    return blocks.map((row) =>
      row.map((block) => (block ? { ...block, sprite: null as any } : null))
    );
  }
   ...
   * 隣接する同色ブロックを取得（さめがめルール）
   */
  private static getConnectedBlocks(
    blocks: Block[][],
    startX: number,
    startY: number
  ): { x: number; y: number }[] {
