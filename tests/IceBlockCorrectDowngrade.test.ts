...
describe('氷結ブロックの正しい挙動テスト', () => {
  it('氷結ブロックLv1は通常ブロックにダウングレードされるべき', () => {
    ...
    // 消去可能なブロックグループを取得
    const group = blockLogic.findMatchingGroup(blocks, 0, 0);
  ...
  it('通常ブロックと氷結ブロックが混在する場合、氷結ブロックはダウングレードされ、通常ブロックのみ消去されるべき', () => {
    ...
    // 通常ブロックのみを消去
    updatedBlocks = blockLogic.removeBlocks(updatedBlocks, normalBlocks);
    
    // 元々の通常ブロックと、通常ブロックになった氷結ブロックが消去されるべき
