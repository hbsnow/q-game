...
/**
 * 氷結ブロックの期待される挙動に関するテスト
 * 仕様: 氷結ブロックは消去されるのではなく、レベルが下がるだけ
 */
describe('氷結ブロックの期待される挙動テスト', () => {
  it('氷結ブロックを含むグループを消去する際、氷結ブロックは消去されずレベルダウンするべき', () => {
    ...
    //     a   b   c
    //  +-----------+
    // 0|     __R _*R |  <- 元々の通常ブロックのみ消去
    // 1| __Y __B __G |
    //  +-----------+
    expect(finalBlocks[0][0]).toBeNull(); // 元々の通常ブロックは消去
    expect(finalBlocks[0][1]).not.toBeNull(); // 氷結Lv1から変わった通常ブロックは残る
    expect(finalBlocks[0][1].type).toBe('normal');
    expect(finalBlocks[0][2]).not.toBeNull(); // 氷結Lv1は残っている
